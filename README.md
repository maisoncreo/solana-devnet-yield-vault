...
Первая сборка создаёт собственный ключ программы в `target/deploy/defi_vault-keypair.json`. `anchor keys sync --provider.cluster localnet` переносит его публичный адрес в `declare_id!` и раздел `[programs.localnet]` в `Anchor.toml`. Явное указание `localnet` обязательно для этой инструкции: синхронизация другого кластера может оставить старый адрес локальной программы и привести к `DeclaredProgramIdMismatch` при тестировании. Повторная сборка согласует бинарный файл и IDL с этим адресом. Изменение Program ID здесь ожидаемо и относится только к этой локальной копии; опубликованный Devnet Vault не меняется.
...
Результаты подтверждены предоставленным пользователем выводом команд и скриншотами. При проверке обнаружено несовпадение Program ID: добавление `--provider.cluster localnet` к команде синхронизации и повторная сборка устранили его. Исправленная команда включена в инструкцию выше.

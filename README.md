...
Локальный MVP собран: 10 Rust-тестов и 19 транзакционных тестов прошли, дополнительно 6 проверок числового адаптера; React production build выполнен. Подробности — в `STATUS.md`. Program ID опубликованной программы: `C6BADkFFFckPvxBCzsedCGFVzF9t1ByZszkUTmWjHbzR`. Программа развёрнута в Devnet. Все демонстрационные транзакции получили статус finalized.
...
Самый короткий путь — открыть [демоверсию](https://talbek-devnet-yield-vault.aikgjpch.chatgpt.site) в браузере с Phantom или другим совместимым Solana-кошельком. Включите Solana Devnet. Для операций нужны тестовые SOL на комиссии и DEV-USDC mint проекта, указанного ниже.
...
Проверенное окружение: macOS, Rust/Cargo 1.96.1, Solana CLI 4.2.2, Anchor CLI 0.31.1, Node.js 22.14.0, npm 10.9.2. На Intel macOS для Solana CLI может потребоваться `brew install libusb`.
...
Инструкции проверены в отдельном клоне GitHub-репозитория с установленными инструментами разработки. Выполнены установка зависимостей, сборка программы, подготовка localnet, тесты и сборка интерфейса. Локальный ключ программы, `declare_id!`, `[programs.localnet]` и IDL согласованы; `[programs.devnet]` сохраняет опубликованный адрес.

| Проверка | Результат |
|---|---|
| Rust | 10 тестов пройдено |
| Интеграционные тесты | 19 тестов пройдено |
| Числовой адаптер | 6 тестов пройдено |
| TypeScript | Без ошибок |
| Сборка frontend | Успешно |
| npm audit, корень и frontend | 0 high, 0 critical |

[Протокол проверки](audit/verification/README.md) содержит проверенный коммит и вывод команд. Установка на чистую ОС и повторное развёртывание в Devnet в эту проверку не входили. Разбор зависимостей и оставшихся замечаний: [DEPENDENCY_AUDIT.md](DEPENDENCY_AUDIT.md).
...
Депозит и вывод выполнены через Phantom. Обе транзакции имеют статус finalized:
...
- [Результаты проверки](STATUS.md)
- [Отчёт по зависимостям](DEPENDENCY_AUDIT.md)
- [Устранение замечаний](REVIEW_RESPONSE.md)
- [Контрольный список](AUDIT_CHECKLIST.md)

# Checklist перед сдачей

Этот список не является независимым аудитом. Галочка означает наличие в коде, а не доказательство отсутствия ошибок.

- [x] Signer для пользователя и администратора.
- [x] Vault seeds привязаны к admin и mint.
- [x] Position seeds привязаны к vault и user.
- [x] SPL mint, token authority, program owner проверяются типами и constraints.
- [x] Token account vault имеет фиксированные seeds.
- [x] Нулевые суммы, capacity, недостаточные shares, dust, min output проверяются.
- [x] Проверяемая арифметика u64/u128.
- [x] add_yield доступен только admin и требует непустого vault.
- [x] Mint с freeze authority не допускается.
- [x] Повторный депозит сохраняет существующие доли.
- [x] Ключи и target исключены через .gitignore.
- [x] Успешная SBF-сборка и IDL.
- [x] Пройдены Rust и транзакционные тесты (10 + 19), дополнительно 6 тестов числового адаптера.
- [x] Тесты чужого admin signer, mint и token account.
- [x] Подмена чужой позиции: отказ и неизменность обоих балансов/позиций.
- [x] Отсутствующая подпись и подмена signer-флага отклоняются.
- [x] Два независимых вкладчика, доходность, частичный/полный вывод и сохранение суммы активов.
- [ ] Fuzz/property-тесты.
- [x] Проверен frontend build и открытие wallet modal.
- [x] Депозит и вывод через Phantom в Devnet подтверждены (finalized).
- [x] High findings разобраны в DEPENDENCY_AUDIT.md; toml и serialize-javascript обновлены.
- [x] Нативный bigint-buffer заменён локальным JS-адаптером, проверена совместимость; high/critical = 0.
- [ ] Устранены все moderate/low и выполнен независимый аудит локального адаптера.
- [x] Devnet deployment подтверждён, Program ID и 6 finalized Explorer-ссылок сохранены.
- [x] Публичный GitHub-репозиторий создан; ключи, target и node_modules исключены.
- [x] Демо размещено в Sites, публичный доступ открыт.
- [x] Доступ преподавателю по публичной ссылке.
- [ ] Upgrade authority и ограничения объяснены в демонстрации.

...
Вместо нативного пакета используется локальный адаптер `bigint-buffer` (приватная версия проекта `1.1.6`, не upstream-релиз) в `vendor/bigint-buffer`, установленный под совместимым dependency name. В корне и frontend используются локальная зависимость `file:` и override `$bigint-buffer`. Реализация сопровождается в репозитории проекта. Нативный код, bindings и установочный скрипт сборки отсутствуют.
...
Шесть тестов `npm run test:codec` охватывают известные значения, порядок байтов, пустые буферы, случайные значения, максимальные значения, переполнение, неверные типы, превышение длины и фактическое подключение адаптера через SPL в корне и frontend. Тесты против локального validator проверяют реальные SPL инструкции и состояния.

**Итоговый audit: high = 0, critical = 0 в обеих установках.** Это не независимый аудит локального адаптера. При обновлении SPL-зависимостей требуется повторная проверка совместимости адаптера. Переход на исправленную upstream-реализацию возможен после её проверки.

## Остальные замечания

Остаются moderate/low в цепочках `uuid`, `stream-json`, `diff` и их родителей (`jayson`, `mocha`, SPL, Anchor) и кошельковых адаптеров. Полные списки сохранены в `audit/npm-root.json` и `audit/npm-frontend.json`. В число moderate входят транзитивные родительские пакеты. npm также выдаёт peer-dependency warnings для транзитивных wallet/mobile/React Native пакетов. Проверка TypeScript и Vite build проходит; мобильный React Native сценарий не проверен.
